...
function Details(props) {
    return (  
        ...
        <div
          ...
          style={{ backgroundImage: `url(${props.img_org + props.backdrop_path})` }}
        >
          <Container>
            <Row className="d-flex justify-content-center align-items-center flex-md-row flex-column">
              ...
              <Col md={8}>
                <div className={props.classNameDetailsContent}>
                  <p className={props.classNameDetailsTitle}>{props.original_title}</p>
                  <p>{props.overview}</p>
                  <ul className="d-flex">
                    {props.genres.map(()=>(
                      <li className={props.classNameGenres} key={props.genresid}>{props.genresname}</li>
                    ))}
                  </ul>
